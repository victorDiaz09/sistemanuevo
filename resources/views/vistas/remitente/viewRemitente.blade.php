...
@section('content')
    ...
    <h4 class="text-center text-secondary">DATOS DEL REMITENTE</h4>
    ...
    @foreach ($sql as $datos)
        <div class="modal-body bg-light">
            <form action="{{ route('remitente.update', $datos->id_remitente) }}" method="POST" id="formActualizar">
                @csrf
                @method('PUT')
            ...
            <div class="text-right p-2 d-flex justify-content-end flex-wrap gap-2 col-12">
                <a href="{{ route('remitente.index') }}" class="btn btn-secondary"><i class="fas fa-caret-left"></i>
                    Atras</a>


                <form action="{{ route('remitente.destroy', $datos->id_remitente) }}" method="POST"
                    class="d-inline formulario-eliminar">
                    @csrf
                    @method('DELETE')
                </form>
                <a class="btn btn-danger eliminar" data-id="{{ $datos->id_remitente }}"><i class="fas fa-trash-alt"></i>
                    Eliminar Registro</a>
